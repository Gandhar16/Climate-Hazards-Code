...
calc_ndws <- function(yr, mn){
  ...
  eabyep_calc <- function(soilcp = scp, soilsat = sst, avail = AVAIL, rain = prc[[1]], evap = ETMAX[[1]]){
  ...
  # Save ERATIO daily files
  for (i in 1:terra::nlyr(ERATIO)) {
    eratio_file <- paste0(out_dir, '/NDWS-', yr, '-', mn, '-', sprintf('%02d', i), '.tif')
    terra::writeRaster(ERATIO[[i]], eratio_file, overwrite = TRUE)
  }
  
  # Clean up
  rm(list=c("prc", "ETMAX", "AVAIL", "watbal", "ERATIO"))
  gc(verbose=F, full=T, reset=T)
}
...
out_dir <- paste0(root, 'NDWS/Daily')
1:nrow(stp) %>%
